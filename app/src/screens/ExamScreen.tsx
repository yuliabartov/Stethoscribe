...
export function ExamScreen() {
  ...
  return (
    <>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: `linear-gradient(180deg,${color.examGradA} 0%,${color.examGradB} 100%)` }}>
        ...
        <div className="scr" style={{ flex: 1, overflow: 'auto', background: color.cream, borderRadius: '30px 30px 0 0', padding: '20px 18px 130px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
            {(state.examCats || []).map((c, idx) => {
              const active = c.status === 'active';
              const done = c.status === 'done';
              const pending = c.status === 'pending';
              const low = c.low && done;
              const val = c.override != null ? c.override : loc(c, 'sample');
              ...
              const valueText = done ? val : active ? t.capturing : '—';
              const badge = active ? t.listening : done ? '✓' : t.waiting;

              return (
                <div key={idx} style={cardStyle}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                    <div style={{ fontSize: 15, fontWeight: 800, color: active ? color.examActiveText : color.ink }}>{loc(c, 'name')}</div>
