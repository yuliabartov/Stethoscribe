...
- **`app/`** — a working React + Vite + TypeScript implementation of "Stethoscribe," a hands-free voice-documentation app for doctors. This is a **UI prototype**, not the real product: it is a faithful port of an interactive Codex Design mockup (`.dc.html`), built to demonstrate the full bilingual (English/Hebrew, with RTL) UX across all 8 screens with realistic sample data and simulated interactions.
...
A `.Codex/launch.json` at the repo root is already configured for Codex's preview tooling (`preview_start` with name `stethoscribe-web`), pointing at `npm run dev --prefix app` on port 5173.
